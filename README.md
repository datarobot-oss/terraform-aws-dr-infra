...
Two node groups are created by default in the `kubernetes_node_groups` variable:
- A `r-4x` node group intended to host the majority of the DataRobot pods
- A `g4dn-2x` node group intended to host GPU workload pods

Node groups can be configured with any options available in https://github.com/terraform-aws-modules/terraform-aws-eks/tree/master/modules/eks-managed-node-group.

By default, the node groups are configured such that they support cluster-autoscaler scale to/from 0. That is, a copy of each node group is created in each availability zone each autoscaling group is tagged with the appropriate `node-template` tags. This behavior can be toggled on/off using the `kubernetes_node_groups_scale_from_zero` variable.
...
| ---- | ----------- | ---- | ------- | :------: |
| <a name="input_app_identity_name"></a> [app\_identity\_name](#input\_app\_identity\_name) | Name of the IAM role for the DataRobot application. Defaults to <name>-app-irsa. | `string` | `null` | no |
...
| <a name="input_cluster_autoscaler_version"></a> [cluster\_autoscaler\_version](#input\_cluster\_autoscaler\_version) | Version of the cluster-autoscaler helm chart to install | `string` | `"9.51.0"` | no |
| <a name="input_container_registry_create_repository_policy"></a> [container\_registry\_create\_repository\_policy](#input\_container\_registry\_create\_repository\_policy) | Indicates whether to create a repository policy for the ECR repositories. Ignored if create\_container\_registry is false. | `bool` | `true` | no |
| <a name="input_container_registry_repo_prefix"></a> [container\_registry\_repo\_prefix](#input\_container\_registry\_repo\_prefix) | Prefix to apply to created repositories. If not specified, the `name` variable will be used as the prefix. Ignored if create\_container\_registry is false. | `string` | `null` | no |
| <a name="input_container_registry_repos"></a> [container\_registry\_repos](#input\_container\_registry\_repos) | Repositories to create. Ignored if create\_container\_registry is false. | `set(string)` | <pre>[<br/>  "base-image",<br/>  "custom-apps-managed-image",<br/>  "custom-jobs/managed-image",<br/>  "ephemeral-image",<br/>  "managed-image",<br/>  "services/custom-model-conversion",<br/>  "spark-batch-image",<br/>  "workload-api/managed-workloads"<br/>]</pre> | no |
...
| <a name="input_create_observability"></a> [create\_observability](#input\_create\_observability) | Whether to provision observability resources with Cloudwatch, Prometheus and Grafana | `bool` | `false` | no |
...
| <a name="input_ingress_nginx_use_acm_cert"></a> [ingress\_nginx\_use\_acm\_cert](#input\_ingress\_nginx\_use\_acm\_cert) | Use ACM certificate on the ingress NLB to terminate TLS. Only applicable when `create_acm_certificate` or `existing_acm_certificate_arn` is set. | `bool` | `true` | no |
...
| <a name="input_kubernetes_iam_role_use_name_prefix"></a> [kubernetes\_iam\_role\_use\_name\_prefix](#input\_kubernetes\_iam\_role\_use\_name\_prefix) | Determines whether the IAM role name (`kubernetes_iam_role_name`) is used as a prefix | `bool` | `false` | no |
| <a name="input_kubernetes_node_groups"></a> [kubernetes\_node\_groups](#input\_kubernetes\_node\_groups) | Map of EKS managed node groups. See https://github.com/terraform-aws-modules/terraform-aws-eks/tree/master/modules/eks-managed-node-group for further configuration options. | `any` | <pre>{<br/>  "g4dn-2x": {<br/>    "ami_type": "AL2023_x86_64_NVIDIA",<br/>    "block_device_mappings": {<br/>      "xvda": {<br/>        "device_name": "/dev/xvda",<br/>        "ebs": {<br/>          "encrypted": true,<br/>          "volume_size": 200,<br/>          "volume_type": "gp3"<br/>        }<br/>      }<br/>    },<br/>    "create": true,<br/>    "desired_size": 0,<br/>    "iam_role_use_name_prefix": false,<br/>    "instance_types": [<br/>      "g4dn.2xlarge"<br/>    ],<br/>    "labels": {<br/>      "datarobot.com/gpu-type": "nvidia-t4-2x",<br/>      "datarobot.com/node-capability": "gpu"<br/>    },<br/>    "max_size": 10,<br/>    "min_size": 0,<br/>    "taints": {<br/>      "nvidia_gpu": {<br/>        "effect": "NO_SCHEDULE",<br/>        "key": "nvidia.com/gpu",<br/>        "value": "true"<br/>      }<br/>    },<br/>    "use_name_prefix": false<br/>  },<br/>  "r-4x": {<br/>    "ami_type": "AL2023_x86_64_STANDARD",<br/>    "block_device_mappings": {<br/>      "xvda": {<br/>        "device_name": "/dev/xvda",<br/>        "ebs": {<br/>          "encrypted": true,<br/>          "volume_size": 200,<br/>          "volume_type": "gp3"<br/>        }<br/>      }<br/>    },<br/>    "create": true,<br/>    "desired_size": 1,<br/>    "iam_role_use_name_prefix": false,<br/>    "instance_types": [<br/>      "r6a.4xlarge",<br/>      "r6i.4xlarge",<br/>      "r5.4xlarge",<br/>      "r4.4xlarge"<br/>    ],<br/>    "labels": {<br/>      "datarobot.com/node-capability": "cpu"<br/>    },<br/>    "max_size": 10,<br/>    "min_size": 0,<br/>    "taints": {},<br/>    "use_name_prefix": false<br/>  }<br/>}</pre> | no |
| <a name="input_kubernetes_node_groups_scale_from_zero"></a> [kubernetes\_node\_groups\_scale\_from\_zero](#input\_kubernetes\_node\_groups\_scale\_from\_zero) | Allow cluster-autoscaler to scale node groups to/from zero. This creates copies of each node group in each availability zone and adds node-template tags to the node group autoscaling groups. More info: https://github.com/kubernetes/autoscaler/blob/master/cluster-autoscaler/cloudprovider/aws/README.md#Auto-discovery-setup | `bool` | `true` | no |
...
| <a name="input_kyverno_notation_aws_signer_profile_arn"></a> [kyverno\_notation\_aws\_signer\_profile\_arn](#input\_kyverno\_notation\_aws\_signer\_profile\_arn) | ARN of the signer profile to use for image signature verification with kyverno-notation-aws. Required if kyverno\_notation\_aws is true. | `string` | `null` | no |
...
| <a name="input_kyverno_policy_failure_policy"></a> [kyverno\_policy\_failure\_policy](#input\_kyverno\_policy\_failure\_policy) | Kyverno policy failure policy. Determines what action to take when a webhook fails to respond. | `string` | `"Fail"` | no |
| <a name="input_kyverno_policy_validation_failure_action"></a> [kyverno\_policy\_validation\_failure\_action](#input\_kyverno\_policy\_validation\_failure\_action) | Kyverno policy validation failure action. Determines whether Kyverno will enforce policies or just report violations when a policy validation failure occurs. | `string` | `"Enforce"` | no |
...
| <a name="input_mongodb_atlas_autoscaling_max_instance_size"></a> [mongodb\_atlas\_autoscaling\_max\_instance\_size](#input\_mongodb\_atlas\_autoscaling\_max\_instance\_size) | Maximum instance size when compute autoscaling is enabled | `string` | `"M80"` | no |
| <a name="input_mongodb_atlas_autoscaling_min_instance_size"></a> [mongodb\_atlas\_autoscaling\_min\_instance\_size](#input\_mongodb\_atlas\_autoscaling\_min\_instance\_size) | Minimum instance size when compute autoscaling is enabled | `string` | `"M10"` | no |
...
| <a name="input_mongodb_atlas_private_key"></a> [mongodb\_atlas\_private\_key](#input\_mongodb\_atlas\_private\_key) | Private API key for Mongo Atlas | `string` | `"atlas-private-key"` | no |
| <a name="input_mongodb_atlas_public_key"></a> [mongodb\_atlas\_public\_key](#input\_mongodb\_atlas\_public\_key) | Public API key for Mongo Atlas | `string` | `"atlas-public-key"` | no |
...
| <a name="input_mongodb_backup_schedule"></a> [mongodb\_backup\_schedule](#input\_mongodb\_backup\_schedule) | Configuration for the MongoDB Atlas cloud backup schedule policy items and cross-region copy settings | <pre>object({<br/>    policy_item_hourly = optional(object({<br/>      frequency_interval = optional(number, 6)<br/>      retention_unit     = optional(string, "days")<br/>      retention_value    = optional(number, 7)<br/>    }), {})<br/>    policy_item_daily = optional(object({<br/>      frequency_interval = optional(number, 1)<br/>      retention_unit     = optional(string, "days")<br/>      retention_value    = optional(number, 30)<br/>    }), {})<br/>    policy_item_weekly = optional(object({<br/>      frequency_interval = optional(number, 6)<br/>      retention_unit     = optional(string, "days")<br/>      retention_value    = optional(number, 30)<br/>    }), {})<br/>    policy_item_monthly = optional(object({<br/>      frequency_interval = optional(number, 1)<br/>      retention_unit     = optional(string, "months")<br/>      retention_value    = optional(number, 1)<br/>    }), {})<br/>    copy_settings = optional(object({<br/>      enabled            = optional(bool, true)<br/>      cloud_provider     = optional(string, "AWS")<br/>      frequencies        = optional(list(string), ["DAILY"])<br/>      should_copy_oplogs = optional(bool, false)<br/>    }), {})<br/>  })</pre> | `{}` | no |
| <a name="input_mongodb_enable_mongoatlas_autoscaling"></a> [mongodb\_enable\_mongoatlas\_autoscaling](#input\_mongodb\_enable\_mongoatlas\_autoscaling) | Enable compute autoscaling on the MongoDB Atlas cluster | `bool` | `true` | no |
...
| <a name="input_mongodb_name"></a> [mongodb\_name](#input\_mongodb\_name) | Name of the MongoDB Atlas instance. If not specified, the `name` variable will be used. | `string` | `null` | no |
...
| <a name="input_network_endpoints"></a> [network\_endpoints](#input\_network\_endpoints) | A map of interface and/or gateway endpoints containing their properties and configurations. See https://github.com/terraform-aws-modules/terraform-aws-vpc/blob/master/modules/vpc-endpoints/variables.tf#L19 for available options. If `private_dns_enabled` is `false`, `custom_private_dns_name` and `custom_private_dns_zone` can be used to create a private DNS record for the endpoint. | <pre>list(object({<br/>    service                  = optional(string)<br/>    service_name             = optional(string)<br/>    service_type             = optional(string, "Interface")<br/>    private_dns_enabled      = optional(bool, true)<br/>    custom_private_dns_zone  = optional(string)<br/>    custom_private_dns_name  = optional(string)<br/>    custom_private_dns_type  = optional(string, "CNAME")<br/>    custom_private_dns_ttl   = optional(number, 60)<br/>    custom_private_dns_alias = optional(bool, false)<br/>    subnet_ids               = optional(list(string))<br/>  }))</pre> | <pre>[<br/>  {<br/>    "service": "s3",<br/>    "service_type": "Gateway"<br/>  },<br/>  {<br/>    "service": "ec2"<br/>  },<br/>  {<br/>    "service": "ecr.api"<br/>  },<br/>  {<br/>    "service": "ecr.dkr"<br/>  },<br/>  {<br/>    "service": "logs"<br/>  },<br/>  {<br/>    "service": "sts"<br/>  },<br/>  {<br/>    "service": "elasticloadbalancing"<br/>  },<br/>  {<br/>    "service": "autoscaling"<br/>  },<br/>  {<br/>    "service": "secretsmanager"<br/>  },<br/>  {<br/>    "service": "route53resolver"<br/>  },<br/>  {<br/>    "service": "bedrock-runtime"<br/>  }<br/>]</pre> | no |
...
| <a name="input_network_firewall_delete_protection"></a> [network\_firewall\_delete\_protection](#input\_network\_firewall\_delete\_protection) | Enable delete protection for the AWS Network Firewall | `bool` | `false` | no |
| <a name="input_network_firewall_flow_log_retention"></a> [network\_firewall\_flow\_log\_retention](#input\_network\_firewall\_flow\_log\_retention) | Number of days to retain NFW flow logs. Set to `0` to keep logs indefinitely | `number` | `7` | no |
| <a name="input_network_firewall_policy_stateful_rule_group_reference"></a> [network\_firewall\_policy\_stateful\_rule\_group\_reference](#input\_network\_firewall\_policy\_stateful\_rule\_group\_reference) | Set of configuration blocks containing references to the stateful rule groups that are used in the policy. See [Stateful Rule Group Reference](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/networkfirewall_firewall_policy#stateful-rule-group-reference) for details | <pre>map(object({<br/>    deep_threat_inspection = optional(bool)<br/>    override = optional(object({<br/>      action = optional(string)<br/>    }))<br/>    priority     = optional(number)<br/>    resource_arn = string<br/>  }))</pre> | `null` | no |
| <a name="input_network_firewall_policy_stateless_default_actions"></a> [network\_firewall\_policy\_stateless\_default\_actions](#input\_network\_firewall\_policy\_stateless\_default\_actions) | Set of actions to take on a packet if it does not match any of the stateless rules in the policy. | `list(string)` | <pre>[<br/>  "aws:pass"<br/>]</pre> | no |
...
| <a name="input_postgres_create_route53_cname_record"></a> [postgres\_create\_route53\_cname\_record](#input\_postgres\_create\_route53\_cname\_record) | Create `postgres` CNAME in private zone | `bool` | `true` | no |
...
| <a name="input_postgres_egress_with_cidr_blocks"></a> [postgres\_egress\_with\_cidr\_blocks](#input\_postgres\_egress\_with\_cidr\_blocks) | List of egress rules to create for the PostgreSQL security group where 'cidr\_blocks' is used. See https://github.com/terraform-aws-modules/terraform-aws-security-group/blob/master/README.md#input_egress_with_cidr_blocks for more details. | `list(map(string))` | `[]` | no |
| <a name="input_postgres_engine_version"></a> [postgres\_engine\_version](#input\_postgres\_engine\_version) | The engine version to use | `string` | `"17"` | no |
| <a name="input_postgres_family"></a> [postgres\_family](#input\_postgres\_family) | Postgres family variable to support major version upgrades | `string` | `"postgres17"` | no |
| <a name="input_postgres_final_snapshot_identifier_prefix"></a> [postgres\_final\_snapshot\_identifier\_prefix](#input\_postgres\_final\_snapshot\_identifier\_prefix) | Prefix for the final snapshot identifier when postgres\_skip\_final\_snapshot is false | `string` | `"final"` | no |
...
| <a name="input_postgres_name"></a> [postgres\_name](#input\_postgres\_name) | Name of the RDS postgres instance. If not specified, the `name` variable will be used. | `string` | `null` | no |
| <a name="input_postgres_parameters"></a> [postgres\_parameters](#input\_postgres\_parameters) | Parameters to add to the Postgres parameter group | <pre>list(object({<br/>    name         = string<br/>    value        = string<br/>    apply_method = optional(string)<br/>  }))</pre> | <pre>[<br/>  {<br/>    "name": "password_encryption",<br/>    "value": "scram-sha-256"<br/>  }<br/>]</pre> | no |
| <a name="input_postgres_skip_final_snapshot"></a> [postgres\_skip\_final\_snapshot](#input\_postgres\_skip\_final\_snapshot) | Whether to skip the final snapshot before the DB instance is deleted | `bool` | `true` | no |
| <a name="input_postgres_subnet_group_name"></a> [postgres\_subnet\_group\_name](#input\_postgres\_subnet\_group\_name) | Name of RDS postgres instance DB subnet group. | `string` | `null` | no |
| <a name="input_postgres_subnet_group_use_name_prefix"></a> [postgres\_subnet\_group\_use\_name\_prefix](#input\_postgres\_subnet\_group\_use\_name\_prefix) | Determines whether to use subnet\_group\_name as is or create a unique name beginning with the subnet\_group\_name as the prefix | `bool` | `true` | no |
...
| <a name="input_redis_engine_version"></a> [redis\_engine\_version](#input\_redis\_engine\_version) | The Elasticache engine version to use | `string` | `"7.0"` | no |
| <a name="input_redis_name"></a> [redis\_name](#input\_redis\_name) | Name of the Elasticache Redis instance. If not specified, the `name` variable will be used. | `string` | `null` | no |
...
| <a name="input_s3_bucket_kms_key_arn"></a> [s3\_bucket\_kms\_key\_arn](#input\_s3\_bucket\_kms\_key\_arn) | ARN of a KMS key used to encrypt the S3 storage bucket. The DataRobot application is granted permission to use this key. When create\_storage is true, the created bucket is configured to use this key for default server-side encryption (SSE-KMS) instead of SSE-S3 (AES256). Unlike the other storage variables, this is also applied when an existing\_s3\_bucket\_id is specified, so that the application can read and write objects in an externally managed bucket encrypted with a customer managed key. | `string` | `null` | no |
...
| <a name="output_ingress_lb_arn_suffix"></a> [ingress\_lb\_arn\_suffix](#output\_ingress\_lb\_arn\_suffix) | The ARN suffix of the ingress load balancer created by ingress-nginx |
...
| <a name="output_network_endpoints"></a> [network\_endpoints](#output\_network\_endpoints) | Map of created VPC endpoints keyed by service name. |
